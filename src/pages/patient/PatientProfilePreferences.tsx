...
const Pill = ({
  ...
  <button
    ...
    style={{
      ...
      borderRadius: "10px",
      padding: sub ? "0.55rem 0.9rem" : "0.45rem 0.9rem",
  ...
  >
    <span style={{ fontSize: "0.82rem", fontWeight: selected ? 600 : 500, color: selected ? color : "#334155", fontFamily: "'DM Sans', sans-serif" }}>{label}</span>
    {sub && <span style={{ fontSize: "0.72rem", color: "#94a3b8", fontFamily: "'DM Sans', sans-serif" }}>{sub}</span>}
  </button>
);
...
export default function PatientProfilePreferences() {
  ...
  useEffect(() => {
    patientApi.getPreferences().catch(() => null).then((res: any) => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await patientApi.savePreferences(prefs as unknown as Record<string, unknown>);
      setSaved(true);
      // Navigate to sessions after short delay so user sees the success state
      setTimeout(() => navigate('/patient/sessions', { replace: true }), 900);
    } catch (err) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#f4f7fb", minHeight: "100vh", padding: "0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .pref-tab-btn:hover { background: rgba(255,255,255,0.2) !important; }
      `}</style>

      {/* Sticky Header — centered rounded card */}
      <div style={{
        ...
        <div style={{
          ...
          boxShadow: "0 4px 20px rgba(0,35,101,0.18)",
        }}>
          {/* Title row */}
          <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.85rem" }}>
            ...
            <div>
              <div style={{ fontSize: "0.97rem", fontWeight: 700, color: "#fff", lineHeight: 1.3 }}>My therapy preferences</div>
              <div style={{ fontSize: "0.72rem", color: "rgba(255,255,255,0.6)", marginTop: "1px" }}>Helps us match you to the right provider</div>
            </div>
          </div>
            ...
            {tabs.map((tab, i) => (
              <button
                ...
                style={{
                  ...
                  color: activeTab === i ? "#002365" : "rgba(255,255,255,0.85)",
                  cursor: "pointer",
                  whiteSpace: "nowrap",
                  transition: "all 0.15s",
                  fontFamily: "'DM Sans', sans-serif",
      ...
      {/* Content */}
      <div style={{ maxWidth: "680px", margin: "0 auto", padding: "1rem 1rem 2rem" }}>
          ...
          <>
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {LANGUAGES.map((lang) => (
                  <button key={lang.code} onClick={() => set("primaryLanguage", lang.code)} style={{ border: prefs.primaryLanguage === lang.code ? "2px solid #002365" : "1.5px solid #e2e8f0", borderRadius: "10px", padding: "0.65rem 1rem", background: prefs.primaryLanguage === lang.code ? "#002365" : "#fff", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: "2px", minWidth: "90px", transition: "all 0.15s" }}>
                    <span style={{ fontSize: "1rem", fontWeight: 700, color: prefs.primaryLanguage === lang.code ? "#fff" : "#334155", fontFamily: "'DM Sans', sans-serif" }}>{lang.script}</span>
                    <span style={{ fontSize: "0.68rem", color: prefs.primaryLanguage === lang.code ? "rgba(255,255,255,0.75)" : "#94a3b8", fontFamily: "'DM Sans', sans-serif" }}>{lang.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {LANGUAGES.filter((l) => l.code !== prefs.primaryLanguage).map((lang) => (
                  <Tag key={lang.code} label={`${lang.script} · ${lang.label}`} selected={prefs.secondaryLanguages.includes(lang.code)} onClick={() => toggle("secondaryLanguages", lang.code)} />
                ))}
              </div>
            </div>
              ...
              <div style={{ marginBottom: "1rem" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                  {[{ id: "whatsapp", label: "WhatsApp" }, { id: "sms", label: "SMS" }, { id: "email", label: "Email (optional)" }].map((ch) => (
                    <Tag key={ch.id} label={ch.label} selected={prefs.reminderChannels.includes(ch.id)} onClick={() => toggle("reminderChannels", ch.id)} />
                  ))}
                </div>
              </div>
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                  {[{ id: "24h", label: "24 hrs before" }, { id: "2h", label: "2 hrs before" }, { id: "1h", label: "1 hr before" }, { id: "15m", label: "15 min before" }].map((t) => (
                    <Tag key={t.id} label={t.label} selected={prefs.reminderTiming === t.id} onClick={() => set("reminderTiming", t.id)} />
                  ))}
                </div>
              </div>
            </div>
        ...
        {activeTab === 1 && (
          <>
            <div style={cardStyle}>
              <SectionHeader step={4} title="How do you prefer to connect?" sub="Select all that work for you — provider will confirm availability" />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.65rem" }}>
                {THERAPY_MODES.map((mode) => (
                  <Pill key={mode.id} label={`${mode.icon} ${mode.label}`} sub={mode.sub} selected={prefs.therapyModes.includes(mode.id)} onClick={() => toggle("therapyModes", mode.id)} />
                ))}
              </div>
            </div>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {DURATIONS.map((d) => (
                  <Tag key={d} label={d} selected={prefs.sessionDuration === d} onClick={() => set("sessionDuration", d)} />
                ))}
              </div>
              <div style={{ marginTop: "1rem", padding: "0.75rem", background: "#f0f7ff", borderRadius: "8px", borderLeft: "3px solid #185FA5" }}>
                <div style={{ fontSize: "0.75rem", color: "#185FA5", fontWeight: 600, marginBottom: "2px" }}>Provider note</div>
                <div style={{ fontSize: "0.73rem", color: "#334155" }}>Most first sessions are 50 min. Your provider may suggest adjustments after the initial consultation.</div>
              </div>
            </div>
          </>
        )}
        ...
        {activeTab === 2 && (
          <>
            <div style={cardStyle}>
              ...
              {Object.keys(prefs.availability).some((d) => (prefs.availability[d] || []).length > 0) && (
                <div style={{ marginTop: "1rem", display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                  {Object.entries(prefs.availability).filter(([, slots]) => slots.length > 0).map(([day, slots]) => (
                    <span key={day} style={{ fontSize: "0.7rem", padding: "0.2rem 0.6rem", background: "#002365", color: "#fff", borderRadius: "20px", fontWeight: 600 }}>
            ...
            <div style={cardStyle}>
              ...
              <select value={prefs.timezone} onChange={(e) => set("timezone", e.target.value)} style={{ width: "100%", padding: "0.65rem 0.85rem", borderRadius: "10px", border: "1.5px solid #e2e8f0", fontSize: "0.84rem", color: "#334155", background: "#fff", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", appearance: "none" }}>
              ...
              {prefs.timezone !== "Asia/Kolkata" && (
                <div style={{ marginTop: "0.65rem", padding: "0.65rem", background: "#fef9ee", borderRadius: "8px", borderLeft: "3px solid #BA7517", fontSize: "0.73rem", color: "#633806" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.25rem" }}>
          <button onClick={() => setActiveTab((t) => Math.max(0, t - 1))} disabled={activeTab === 0} style={{ border: "1.5px solid #e2e8f0", borderRadius: "10px", padding: "0.55rem 1.1rem", fontSize: "0.82rem", fontWeight: 600, color: "#475569", background: "#fff", cursor: activeTab === 0 ? "not-allowed" : "pointer", opacity: activeTab === 0 ? 0.4 : 1, fontFamily: "'DM Sans', sans-serif" }}>
            ← Back
          </button>

          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            {tabs.map((_, i) => (
              <button key={i} onClick={() => setActiveTab(i)} style={{ width: i === activeTab ? "20px" : "8px", height: "8px", borderRadius: "4px", background: i === activeTab ? "#002365" : "#cbd5e1", border: "none", cursor: "pointer", transition: "all 0.2s", padding: 0 }} />
            ))}
          </div>

          {activeTab < tabs.length - 1 ? (
            <button onClick={() => setActiveTab((t) => Math.min(tabs.length - 1, t + 1))} style={{ border: "none", borderRadius: "10px", padding: "0.55rem 1.1rem", fontSize: "0.82rem", fontWeight: 700, color: "#fff", background: "#002365", cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
              Next →
            </button>
          ) : (
            <button onClick={handleSave} disabled={saving} style={{ border: "none", borderRadius: "10px", padding: "0.55rem 1.25rem", fontSize: "0.82rem", fontWeight: 700, color: "#fff", background: saved ? "#0F6E56" : "#002365", cursor: saving ? "wait" : "pointer", fontFamily: "'DM Sans', sans-serif", transition: "background 0.2s" }}>
        ...
        {saved && (
          <div style={{ marginTop: "1rem", padding: "0.85rem 1rem", background: "#f0fdf4", borderRadius: "12px", border: "1px solid #bbf7d0", display: "flex", gap: "0.65rem", alignItems: "flex-start" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "0.72rem", color: "#334155", marginTop: "2px" }}>
                <code style={{ background: "#e0f2fe", padding: "1px 5px", borderRadius: "4px", fontSize: "0.69rem", color: "#0c4a6e" }}>PATCH /api/v1/patient/preferences</code>
                {" "}— language, therapy mode and schedule saved. TherapeuticGPS will use these alongside PHQ-9 output to generate provider match scores.
              </div>
            </div>
          </div>
        )}
